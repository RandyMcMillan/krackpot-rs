...
build-server:
	CARGO_TARGET_DIR=$(CARGO_TARGET_DIR) $(CARGO) build --bin krackpot-server --features native
...
site: wasm
	...
	mkdir -p site/krackpot
	cp demo/krackpot/index.html site/krackpot/index.html
	mkdir -p site/krackpot/source
	cp demo/krackpot/source/index.html site/krackpot/source/index.html
	cp -R demo/krackpot/src site/krackpot/
	mkdir -p site/git
...
server: build-server site
	CARGO_TARGET_DIR=$(CARGO_TARGET_DIR) $(CARGO) run --bin krackpot-server --features native
