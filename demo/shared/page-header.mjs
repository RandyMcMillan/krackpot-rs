          ...
          <ul class="nav-links">
            ${navItems
              .map((item) => {
                ...
                const rel = item.rel ? ` rel="${escapeHtml(item.rel)}"` : (item.target ? ' rel="noopener noreferrer"' : '');
                return `<li><a class="nav-link${item.current ? ' current' : ''}" href="${href}"${current}${target}${rel}>${label}</a></li>`;
              })
              .join('')}
          </ul>
        ` : ''}
